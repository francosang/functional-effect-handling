...
object Memoization extends IOApp {
  ...
  def loadEmailTemplates(): IO[EmailTemplates] =
    IO.sleep(2.seconds) *>
      ...
      IO.pure(new EmailTemplates {
        override def buildEmailForClient(
            templateId: String,
            client: Client
        ): Email = {
          if (templateId == "negative-balance")
            Email(
              s"Dear ${client.name}: your account has a negative balance",
              List(client.emailAddress)
            )
          else
            Email(
              s"Dear ${client.name}: there is a problem with your account",
              List(client.emailAddress)
            )
        }
      })
      ...
      clients.traverse_ { client =>
        processClient(client).handleErrorWith {
          case NegativeBalance =>
            val email =
              templates.buildEmailForClient("negative-balance", client)
            sendEmail(email)
          case _ =>
      ...
      processClient(client).handleErrorWith { error =>
        templatesIO.flatMap { templates =>
          error match {
            ...
            case _ =>
              val email =
                templates.buildEmailForClient("generic-error", client)
              sendEmail(email)
          }
        }
      }
  ...
  override def run(args: List[String]): IO[ExitCode] = {
    val batches = List(1, 2)
    val clients = List(
      Client("Leandro", "leandro@mail.com"),
      Client("Martin", "martin@mail.com")
    )
    loadEmailTemplates().memoize
      .flatMap { templatesIO =>
        batches
          .traverse_ { it =>
            IO.println(s"Batch: $it") *> processClients3(clients, templatesIO)
          }
      }
      .as(ExitCode.Success)
